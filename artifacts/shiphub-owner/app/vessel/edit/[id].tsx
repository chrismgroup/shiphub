...
export default function EditVessel() {
  ...
  const remove=useMutation({mutationFn:()=>api.vessels.delete(vesselId),onSuccess:()=>{qc.removeQueries({queryKey:['vessel',vesselId]});qc.invalidateQueries({queryKey:['my-vessels']});router.replace('/(tabs)/fleet')},onError:(e:Error)=>Alert.alert('Delete failed',e.message)});
  const confirmDelete=()=>{const message=`This removes ${vessel.data?.name??'this vessel'} and its closed charter history from the marketplace database. This cannot be undone.`;if(Platform.OS==='web'){if(globalThis.confirm(message))remove.mutate();return;}Alert.alert('Delete vessel?',message,[{text:'Cancel',style:'cancel'},{text:'Delete vessel',style:'destructive',onPress:()=>remove.mutate()}]);};
